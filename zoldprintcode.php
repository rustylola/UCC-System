...
// Page footer
    ...
    {   
        ...
        $this->SetTextColor(0);
        $this->SetFont('Arial','B',8);
        $this->Cell(50,6,strtoupper(date("F d, Y")),"",0,'C');
        ...
        $this->SetTextColor(0);
        $this->SetFont('Arial','B',8);
        $this->Cell(50,6,'DATE PRINTED',"T",0,'C');
        ...
        $this->SetTextColor(0);
        $this->SetFont('Arial','B',8);
        $this->Cell(45,6,'SIGNATURE',"T",0,'C');
...
foreach($header as $heading){
    if($heading == 'No'){
        ...
        $pdf->Cell(35,5,$heading,1,0,'C');
    }else if($heading == 'Sex'){
        $pdf->Cell(8,5,$heading,1,0,'C');
    }else if($heading == 'Age'){
        $pdf->Cell(8,5,$heading,1,0,'C');
    }else if($heading == 'Date Vaccinated'){
        $pdf->Cell(29,5,$heading,1,0,'C');
        ...
        if(isset($_GET['selectday'])){
            ...
            if($specificdbdate == $selecteddate && $vaccinationareafilter == 'No Filter'){
                ...
                if($getagefilter == 'No Age'){
                    ...
                    $pdf->Cell(5,6,$no,1);
                    $pdf->Cell(60,6,$fullname,1);
                    $pdf->Cell(8,6,$sex,1,0,'C');
                    $pdf->Cell(8,6,$age,1,0,'C');
                    $pdf->Cell(12,6,$rows['num_dose'],1,0,'C');
                    $pdf->Cell(29,6,date("F d, Y",$datelastvac),1,0,'C');
                    $pdf->Cell(30,6,$rows['vaccine_brand'],1,0,'C');
                    $pdf->Cell(82,6,$rows['vaccination_area'],1,0,'C');
                    $pdf->Cell(40,6,$rows['vaccinated_country'],1,0,'C');
                    $pdf->Cell(25,6,$rows['process_by'],1,0,'C');
                    $pdf->Cell(35,6,date("F d, Y",$dateinput),1,0,'C');

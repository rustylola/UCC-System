...
// Page footer
    ...
    {   
        ...
        $this->SetTextColor(0);
        $this->SetFont('Arial','B',8);
        $this->Cell(100,6,'ACCOUNT NAME AND OVERPRINTEDSIGNATURE',"T",0,'C');
...
$header = array('No','Full Name','Sex','Age','Dose','Date Vaccinated','Vaccine Brand','Venue','Lot No.','Process By','Date Process','Last Update');
...
while($row = mysqli_fetch_assoc($result)){
    ...
    $dateprocessby = $row['date_inputed'];
    $dateprocessbyconvert = strtotime($dateprocessby);
    $dateprocessbyconvertfdy = date("F d, Y",$dateprocessbyconvert);
    ...
    while($rows = mysqli_fetch_assoc($results)){
        ...
        if(isset($_GET['selectday'])){
            ...
            if($dateprocessbyconvertfdy == $selecteddate && $vaccinationareafilter == 'No Filter'){
                ...
                if($getagefilter == 'No Age'){
                    ...
                    if($flags){
                        ...
                        $pdf->Cell(50,$sizename,$fullname,1);
                    ...
                    $pdf->Cell(30,6,$rows['num_dose'].doseidentifier($rows['num_dose'])." ".$rows['vaccine_shot_type'],1,0,'C');
                    ...
                    $pdf->Cell(25,6,$rows['vaccinated_country'],1,0,'C');
                    $pdf->Cell(25,6,$rows['process_by'],1,0,'C');
                    $pdf->Cell(25,6,date("F d, Y",$dateinput),1,0,'C');

                    if($flags){
                        $pdf->Cell(25,$sizename,date("F d, Y",$date),1,0,'C');
                        $flags = false;
                    }else{
                        $pdf->Cell(25);
                    }
                    ...
                    if($flags){
                        $pdf->Cell(25,$sizename,$dateprocessbyconvertfdy,1,0,'C');
                        $flags = false;
                    }else{
                        $pdf->Cell(25);
                    }
            ...
            }else if($dateprocessbyconvertfdy == $selecteddate && $vaccinationareafilter == $row['vaccination_area']){
                if($getagefilter == 'No Age'){
            ...
            if($dateprocessbyconvert > $datestart && $dateprocessbyconvert < $dateend && $vaccinationareafilter == 'No Filter'){
                if($getagefilter == 'No Age'){
            ...
            }else if($dateprocessbyconvert > $datestart && $dateprocessbyconvert < $dateend && $vaccinationareafilter == $row['vaccination_area']){
                if($getagefilter == 'No Age'){
